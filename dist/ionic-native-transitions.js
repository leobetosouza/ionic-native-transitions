    ...
    exports['default'] = function () {
        ...
        function $get($log, $ionicConfig, $rootScope, $timeout) {
            ...
            function transition() {
                ...
                delete options.type;
                $log.debug('[native transition] :D', options);
                var args = arguments[1] || [];
                var successCallback = function(){
                    console.log('successCallback',args)
                    $rootScope.$broadcast('IonicNativeTransitions.finish', args[0], args[1], args[2], args[3], args[4]);
                };
                switch (type) {
                    case 'flip':
                        window.plugins.nativepagetransitions.flip(options, successCallback);
                        break;
                    case 'fade':
                        window.plugins.nativepagetransitions.fade(options, successCallback, successCallback);
                        break;
                    case 'curl':
                        window.plugins.nativepagetransitions.curl(options, successCallback);
                        break;
                    case 'drawer':
                        window.plugins.nativepagetransitions.drawer(options, successCallback);
                        break;
                    case 'slide':
                    default:
                        window.plugins.nativepagetransitions.slide(options, successCallback);
                        break;
                }
            }
